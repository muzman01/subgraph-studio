...
// prettier-ignore
const USDC_WETH_03_POOL = "0x2d774731d831cc3c6a1000fd8f4cefdcd256f955";
...
export function getEthPriceInUSD(): BigDecimal {
  ...
  if (usdcPool !== null) {
    log.info("dai price1", [usdcPool.token0Price.toString()]);
    if (STABLE_IS_TOKEN0 === "true") {
      return usdcPool.token0Price;
    }
    log.info("dai price", [usdcPool.token1Price.toString()]);
...
export function getAdjustedAmounts(
  ...
  let eth = tokenAmount0.times(derivedETH0).plus(tokenAmount1.times(derivedETH1));
  let ethUntracked = eth;
