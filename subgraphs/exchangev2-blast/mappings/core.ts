...
export function handleTransfer(event: Transfer): void {
  ...
  // mints
  let mints = transaction.mints ? transaction.mints : [];
  if (event.params.from.toHex() == ADDRESS_ZERO) {
    ...
    if (mints !== null && (mints.length === 0 || isCompleteMint(mints[mints.length - 1]))) {
  ...
  if (event.params.to.toHex() == pair.id) {
    let burns = transaction.burns ? transaction.burns : [];
    let burnId = event.transaction.hash.toHex().concat("-").concat(BigInt.fromI32(burns.length).toString());
...
export function handleSync(event: Sync): void {
  ...
  token0 = token0 ? token0 : new Token("");
  let token1 = Token.load(pair.token1);
  token1 = token1 ? token1 : new Token("");
...
export function handleMint(event: Mint): void {
  ...
  let mints = transaction.mints ? transaction.mints : [];
  let mint = mints.length > 0 ? MintEvent.load(mints[mints.length - 1]) : null;
  ...
  if (token1.derivedBNB !== null && token0.derivedBNB !== null) {
    amountTotalUSD = token1.derivedBNB
      .times(token1Amount)
  ...
  if (mint === null) {
    mint = new MintEvent(event.transaction.hash.toHex().concat("-").concat(BigInt.fromI32(mints.length).toString()));
  }
...
export function handleBurn(event: Burn): void {
  ...
  let burns = transaction.burns ? transaction.burns : [];
  let burn: BurnEvent | null = burns.length > 0 ? BurnEvent.load(burns[burns.length - 1]) : null;

  let pair = Pair.load(event.address.toHex());
  if (pair === null) {
    pair = new Pair(event.address.toHex());
    pair.token0 = ""; // Token ID atanmalı
    pair.token1 = ""; // Token ID atanmalı
  }
  let pancake = PancakeFactory.load(FACTORY_ADDRESS);
  if (pancake === null) {
    pancake = new PancakeFactory(FACTORY_ADDRESS);
  ...
  if (bundle === null) {
    bundle = new Bundle("1");
    bundle.bnbPrice = ZERO_BD; // Varsayılan değer atanmalı
  }

  let amountTotalUSD =
    token1.derivedBNB !== null && token0.derivedBNB !== null
      ? token1.derivedBNB.times(token1Amount).plus(token0.derivedBNB.times(token0Amount)).times(bundle.bnbPrice)
      : ZERO_BD;
  ...
  if (burn === null) {
    ...
    burn.transaction = transaction.id;
    burn.pair = pair.id;
    burn.liquidity = ZERO_BD; // Varsayılan değer atanmalı
...
export function handleSwap(event: Swap): void {
  ...
  let swaps = transaction.swaps ? transaction.swaps : [];
